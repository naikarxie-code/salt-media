    ...
    >
      ...
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
        ...
        <div ref={textRef} className="flex flex-col gap-6 lg:pt-2">
          ...
          <p className="reveal-line text-base text-neutral-400 leading-relaxed">
            {COMPANY.philosophy}
          </p>
          <div className="reveal-line grid grid-cols-2 gap-4 mt-2">
            <div className="p-5 rounded-2xl border border-neutral-800/80 bg-neutral-950/60 backdrop-blur-xl hover:border-neutral-700 transition-all duration-300">
              ...
              <p className="text-sm text-neutral-300 leading-relaxed">{COMPANY.vision}</p>
            </div>
            <div className="p-5 rounded-2xl border border-neutral-800/80 bg-neutral-950/60 backdrop-blur-xl hover:border-neutral-700 transition-all duration-300">
              <h3 className="text-[10px] uppercase tracking-[0.25em] text-amber-400/90 font-mono mb-1.5">Mission</h3>
              <p className="text-sm text-neutral-300 leading-relaxed">{COMPANY.mission}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {/* Differentiators */}
      <div className="relative z-10 mt-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {COMPANY.differentiators.map((diff, i) => (
          <div
            key={i}
            className="group relative p-6 border border-neutral-800/70 rounded-2xl bg-neutral-950/40 backdrop-blur-md hover:border-amber-500/40 hover:bg-neutral-900/40 transition-all duration-500 shadow-xl overflow-hidden"
          >
            ...
            <p className="text-sm md:text-base text-neutral-300 leading-relaxed font-light">{diff}</p>
          </div>
        ))}
      </div>

      {/* Stats strip */}
      <div className="relative z-10 mt-12 grid grid-cols-2 md:grid-cols-4 gap-6 p-6 rounded-3xl border border-neutral-800/80 bg-neutral-950/60 backdrop-blur-2xl shadow-2xl">
        {[
          { value: STATS.yearsOfCraft, label: "Years of Craft" },
          { value: STATS.brandCollaborations, label: "Brand Partners" },
          { value: STATS.showsProduced, label: "Shows Produced" },
          { value: STATS.crewCapacity, label: "Crew Capacity" },
        ].map((stat, i) => (
          <div key={i} className="flex flex-col justify-center p-4">
            <AnimatedCounter value={stat.value} className="text-3xl md:text-5xl font-bold tracking-tight text-white block" />
            <div className="text-[10px] uppercase tracking-[0.25em] font-mono text-amber-400/80 mt-3">{stat.label}</div>
          </div>
        ))}
      </div>
    </section>
